...
export type { KnockoutFixture } from "../bracket/wc2026KnockoutFixtures.js";
export { WC2026_KNOCKOUT_FIXTURES } from "../bracket/wc2026KnockoutFixtures.js";

async function ensureTbdTeam(tournamentId: number): Promise<number> {
  const existing = await pool.query(
    "SELECT id FROM teams WHERE external_id = 'wc2026-tbd' LIMIT 1"
  );
...
function kickoffUtc(dateLocal: string, timeLocal: string): Date {
  return new Date(`${dateLocal}T${timeLocal}:00-05:00`);
}

export async function importWorldCup2026Knockout(): Promise<{ matches: number }> {
  ...
  await pool.query(
    "DELETE FROM predictions WHERE match_id IN (SELECT id FROM matches WHERE tournament_id = $1 AND stage = 'KNOCKOUT')",
    [tournamentId]
  );
