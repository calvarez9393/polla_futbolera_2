...
export async function recalculateGroupStandings(groupId: number): Promise<void> {
  ...
  for (const row of teamsResult.rows) {
    stats.set(row.team_id as number, {
      played: 0,
      won: 0,
      draw: 0,
      lost: 0,
  ...
  for (const m of matches.rows) {
    const homeId = m.home_team_id as number;
    const awayId = m.away_team_id as number;
