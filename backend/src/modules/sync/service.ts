...
export async function runSync(source = "cron"): Promise<void> {
  ...
  try {
    const [teamsRes, fixturesRes, standingsRes] = await Promise.all([
      withApiRetry(() =>
        footballApi.get("/teams", { params: { league: env.API_FOOTBALL_LEAGUE_ID, season: env.API_FOOTBALL_SEASON } })
      ),
      withApiRetry(() =>
        footballApi.get("/fixtures", { params: { league: env.API_FOOTBALL_LEAGUE_ID, season: env.API_FOOTBALL_SEASON } })
      ),
      withApiRetry(() =>
        footballApi.get("/standings", { params: { league: env.API_FOOTBALL_LEAGUE_ID, season: env.API_FOOTBALL_SEASON } })
      )
    ]);
    ...
    if (fixtureCount === 0) {
      throw new Error(
        `No se recibieron partidos para liga ${env.API_FOOTBALL_LEAGUE_ID} temporada ${env.API_FOOTBALL_SEASON}. En plan gratuito usa temporada 2022–2024.`
      );
    }
    ...
    await pool.query(
      ...
      [env.API_FOOTBALL_LEAGUE_ID, `League ${env.API_FOOTBALL_LEAGUE_ID}`, env.API_FOOTBALL_SEASON]
    );
    ...
    for (const item of teamsRes.data.response ?? []) {
      ...
      await pool.query(
        ...
        SET name = EXCLUDED.name, short_name = EXCLUDED.short_name, logo_url = EXCLUDED.logo_url`,
        [String(team.id), team.name, team.code ?? null, team.logo ?? null]
      );
    }
    ...
    for (const fixtureItem of fixturesRes.data.response ?? []) {
      ...
      if (groupName) {
        const group = await pool.query(
          ...
          RETURNING id`,
          [tournament.id, groupName, groupName]
        );
    ...
    for (const standingsGroup of standingsRes.data.response?.[0]?.league?.standings ?? []) {
      for (const row of standingsGroup) {
        ...
        const group = await pool.query(
          ...
          ON CONFLICT (external_id) DO UPDATE SET name = EXCLUDED.name
          RETURNING id`,
          [tournament.id, row.group, row.group]
        );
        await pool.query(
    ...
    await pool.query(
      "UPDATE sync_runs SET status = 'SUCCESS', finished_at = NOW(), details = details || $2::jsonb WHERE id = $1",
      [
        runId,
        JSON.stringify({
