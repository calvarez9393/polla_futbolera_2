...
function parseArgs() {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (a === "--clean")
            ...
            users = Number(argv[++i]);
        else if (a === "--matchday" && argv[i + 1])
            matchday = Number(argv[++i]);
...
async function cleanCompetitionData(tournamentId) {
    const scores = await pool.query(`DELETE FROM prediction_scores`);
    const preds = await pool.query(`DELETE FROM predictions p
     USING matches m
     WHERE p.match_id = m.id AND m.tournament_id = $1`, [tournamentId]);
    const qualifiers = await pool.query(`DELETE FROM qualifier_predictions`);
    const groupPreds = await pool.query(`DELETE FROM group_predictions gp
     USING groups g
     WHERE gp.group_id = g.id AND g.tournament_id = $1`, [tournamentId]);
    const bonuses = await pool.query(`DELETE FROM bonus_predictions`);
    const matches = await pool.query(`UPDATE matches SET
      status = 'NOT_STARTED',
      home_score = NULL,
      away_score = NULL,
      winner_team_id = NULL,
    ...
    for (const row of groupIds.rows) {
        await recalculateGroupStandings(row.id);
    }
    return {
...
async function ensureUsers(count, passwordHash) {
    const ids = [];
    for (let n = 1; n <= count; n++) {
        const num = String(n).padStart(3, "0");
        const email = `demo${num}@${DEMO_DOMAIN}`;
        const displayName = `Demo ${num}`;
        const result = await pool.query(`INSERT INTO users (email, password_hash, role, display_name, amount_paid)
...
async function getAdminUserIds() {
    const result = await pool.query(`SELECT id, email FROM users WHERE role = 'ADMIN' ORDER BY email`);
    return result.rows.map((r) => ({
        id: r.id,
        email: r.email
    }));
}
async function getGroupMatches(matchday) {
    const tournamentId = await getActiveTournamentId();
    if (!tournamentId) {
        throw new Error("No hay torneo activo. Importa el calendario WC2026 desde Admin.");
    }
    const result = matchday
        ? await pool.query(`SELECT id, matchday, home_team_id, away_team_id, group_id
        FROM matches
        WHERE tournament_id = $1 AND stage = 'GROUP' AND matchday = $2
        ORDER BY matchday ASC, starts_at ASC`, [tournamentId, matchday])
        : await pool.query(`SELECT id, matchday, home_team_id, away_team_id, group_id
        FROM matches
        WHERE tournament_id = $1 AND stage = 'GROUP'
        ORDER BY matchday ASC NULLS LAST, starts_at ASC`, [tournamentId]);
    return result.rows.map((r) => ({
        id: r.id,
        matchday: r.matchday,
...
async function resetGroupMatchesResults(matchIds) {
    const tournamentId = await getActiveTournamentId();
    if (!tournamentId || matchIds.length === 0)
        return;
    await pool.query(`DELETE FROM prediction_scores ps
    WHERE ps.source_type = 'MATCH' AND ps.source_id = ANY($1::bigint[])`, [matchIds]);
    await pool.query(`UPDATE matches SET
      status = 'NOT_STARTED',
      home_score = NULL,
      away_score = NULL,
      winner_team_id = NULL,
      updated_at = NOW()
    WHERE id = ANY($1::bigint[])`, [matchIds]);
    const groupIds = await pool.query(`SELECT DISTINCT group_id FROM matches
    WHERE id = ANY($1::bigint[]) AND group_id IS NOT NULL`, [matchIds]);
    const { recalculateGroupStandings } = await import("../modules/standings/recalculate.js");
    for (const row of groupIds.rows) {
        await recalculateGroupStandings(row.group_id);
    }
}
async function seedPredictions(userIds, matches) {
    const BATCH = 200;
    let inserted = 0;
    const values = [];
    ...
    for (let i = 0; i < values.length; i += BATCH) {
        ...
        await pool.query(`INSERT INTO predictions (user_id, match_id, predicted_home_score, predicted_away_score)
...
async function finalizeMatches(matches) {
    const groupIds = new Set();
    for (const m of matches) {
        const { home, away } = randomScorePair();
        const winnerId = home > away ? m.home_team_id : away > home ? m.away_team_id : null;
        await pool.query(`UPDATE matches SET
        status = 'FINISHED',
        home_score = $1,
        away_score = $2,
        winner_team_id = $3,
        updated_at = NOW()
      WHERE id = $4`, [home, away, winnerId, m.id]);
...
function summarizeByMatchday(matches) {
    const map = new Map();
    for (const m of matches) {
...
async function run() {
    ...
    if (admins.length === 0) {
        console.warn(`⚠️  No hay usuario ADMIN en BD. Crea uno con: npm run seed (${env.ADMIN_EMAIL})`);
    }
    ...
    const predStats = await pool.query(`SELECT COUNT(DISTINCT user_id)::int AS users, COUNT(*)::int AS predictions
    FROM predictions
    WHERE user_id = ANY($1::bigint[]) AND match_id = ANY($2::bigint[])`, [userIds, matchIds]);
