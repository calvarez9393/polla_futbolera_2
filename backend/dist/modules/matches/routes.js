...
matchesRouter.get("/dates", async (_req, res, next) => {
    try {
        ...
        const result = await pool.query(`SELECT ${calDate} AS match_date, COUNT(*)::int AS match_count
      FROM matches
...
matchesRouter.get("/", async (req, res, next) => {
    try {
        ...
        const conditions = [activeTournamentCondition("m", 1)];
      ...
      ORDER BY m.starts_at ASC`, params);
        const enriched = await Promise.all(result.rows.map(async (row) => {
            ...
            return {
                ...
                predictions_open: isMatchAcceptingPredictions(row, lockAt)
            };
        }));
        res.json(enriched);
    }
