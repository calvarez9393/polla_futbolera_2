...
import { knockoutExternalNum, resolveKnockoutAdvancingWithResolvedTeams, resolveKnockoutBracketTeams } from "./knockoutAdvancingResolve.js";
/** Valida y resuelve quién avanza al guardar predicción (usa equipos efectivos del cuadro). */
export async function resolveKnockoutAdvancingForSave(match, userId, predictedHome, predictedAway, predictedAdvancingTeamId) {
    ...
    if (extNum != null && match.external_id) {
        ...
        if (resolved) {
            if (resolved.homeTeamId !== tbdId)
                homeId = resolved.homeTeamId;
            if (resolved.awayTeamId !== tbdId)
                awayId = resolved.awayTeamId;
        }
    }
    return resolveKnockoutAdvancingWithResolvedTeams(predictedHome, predictedAway, predictedAdvancingTeamId, homeId, awayId, tbdId);
}
