...
const THIRD_SLOT_MATCH_ORDER = [74, 77, 79, 80, 81, 82, 85, 87];
 ...
 */
export function assignThirdPlacesToR32Slots(fifa) {
    ...
    const pickCandidate = (allowed, preferBestOnly) => {
        const pools = preferBestOnly
            ? [rankedBest]
            : [rankedBest, rankedAll.filter((t) => !bestThirdIds.has(t.teamId))];
        for (const list of pools) {
            const found = list.find((t) => allowed.has(groupLetterFromName(t.groupName)) && !usedTeamIds.has(t.teamId));
            if (found)
                return found;
        }
        return null;
...
export function describeThirdSlot(matchNum) {
    const fx = WC2026_R32_FIXTURES_RAW.find((f) => f.num === matchNum);
