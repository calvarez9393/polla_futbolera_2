...
export function rankStandings(rows, teamNames) {
...
export function resolveFifaQualifiers(groups, teamNames) {
    const directQualifiers = [];
    const allThirdsRanked = [];
    for (const g of groups) {
        if (g.first)
            directQualifiers.push(g.first);
        if (g.second)
            directQualifiers.push(g.second);
        if (g.third) {
...
export function groupQualificationFromStandings(groupId, groupName, teamIds, matches, teamNames) {
