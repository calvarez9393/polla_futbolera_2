...
adminMatchesRouter.put("/settings/tournament", async (req, res, next) => {
    try {
        ...
        if (input.tournamentName)
            await setSetting("tournament_name", input.tournamentName);
...
adminMatchesRouter.post("/matches", async (req, res, next) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO matches
...
adminMatchesRouter.patch("/matches/:id/result", async (req, res, next) => {
    try {
        const input = z
            ...
            winner_team_id: z.coerce.number().int().positive().nullable().optional()
        })
            .parse(req.body);
        const result = await pool.query(`UPDATE matches SET
        status = $1, home_score = $2, away_score = $3, winner_team_id = $4, updated_at = NOW()
      WHERE id = $5
      RETURNING *`, [input.status, input.home_score, input.away_score, input.winner_team_id, req.params.id]);
        if (!result.rows[0]) {
        ...
        if (input.status === "FINISHED") {
            ...
            const isDraw = input.home_score != null &&
                input.away_score != null &&
                input.home_score === input.away_score;
            if (isKnockout && isDraw && !input.winner_team_id) {
                res.status(400).json({
                    message: "En empate debes indicar el ganador en penales (quién pasa a la siguiente ronda)"
                });
                return;
            }
        ...
        res.json({
            match: result.rows[0],
            message: result.rows[0].stage === "KNOCKOUT"
