...
import { calculateBonusScores, getOfficialBonusResults, setOfficialBonusResults } from "../scoring/bonuses.js";
import { calculateQualifierScores, getOfficialQualifiedTeams, setOfficialQualifiedTeams } from "../scoring/qualifiers.js";
import { importWorldCup2026Knockout } from "../import/worldCup2026Knockout.js";
import { getOfficialStandingsByGroup, syncAllUsersQualifierPredictions, syncOfficialQualifiersFromResults } from "../qualifiers/fromPredictions.js";
import { getActiveTournamentId } from "../settings/service.js";
import { pool } from "../../db/pool.js";
import { calculateExpertDayBonuses, calculateInvictoAndGroupMasterBonuses } from "../scoring/phase1Bonuses.js";
import { applyR16PairingsToMatches, autoFillOfficialR16FromResults, getOfficialR16Bracket, listQualifiedTeamsForR16 } from "../bracket/r16Service.js";
...
adminScoringRouter.put("/official/bonuses", async (req, res, next) => {
    try {
        ...
        await setOfficialBonusResults(input);
        res.json({ ok: true });
    }
    catch (error) {
        next(error);
...
adminScoringRouter.get("/official/context", async (_req, res, next) => {
    try {
        ...
        const teams = tournamentId
            ? (await pool.query(`SELECT t.id, t.name, t.short_name, t.logo_url, g.id AS group_id, g.name AS group_name
            FROM teams t
            JOIN standings s ON s.team_id = t.id AND s.tournament_id = $1
            JOIN groups g ON g.id = s.group_id
            ORDER BY g.name, t.name`, [tournamentId])).rows.map((r) => ({
                id: Number(r.id),
                name: r.name,
