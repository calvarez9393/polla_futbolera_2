...
import { isMatchAcceptingPredictions, resolvePredictionLockAt } from "../settings/service.js";
...
adminUsersRouter.post("/", async (req, res, next) => {
    try {
        ...
        const result = await pool.query(`INSERT INTO users (email, password_hash, role, display_name, amount_paid, payment_notes)
      VALUES ($1, $2, $3, $4, $5, $6)
...
adminUsersRouter.patch("/:id", async (req, res, next) => {
    try {
        ...
        if (sets.length > 0) {
            params.push(userId);
            result = await pool.query(`UPDATE users SET ${sets.join(", ")} WHERE id = $${params.length}
        RETURNING id, email, display_name, role, amount_paid, payment_notes, created_at`, params);
        }
        else {
            result = await pool.query("SELECT id, email, display_name, role, amount_paid, payment_notes, created_at FROM users WHERE id = $1", [userId]);
        }
        if (!result.rows[0]) {
        ...
        const row = result.rows[0];
        const stats = await pool.query(`SELECT COALESCE(SUM(points), 0)::int AS total_points,
...
adminUsersRouter.put("/:userId/predictions", async (req, res, next) => {
    try {
        ...
        if (!isMatchAcceptingPredictions(match, lockAt)) {
            res.status(400).json({
                message: match.status === "FINISHED"
                    ? "El partido ya finalizó; no se pueden modificar predicciones"
                    : "Predicciones cerradas para este partido"
            });
            return;
        }
        const result = await pool.query(`INSERT INTO predictions (user_id, match_id, predicted_home_score, predicted_away_score)
      VALUES ($1, $2, $3, $4)
      ...
      DO UPDATE SET
        ...
        updated_at = NOW()
      RETURNING *`, [userId, input.matchId, input.predictedHomeScore, input.predictedAwayScore]);
        res.json({
