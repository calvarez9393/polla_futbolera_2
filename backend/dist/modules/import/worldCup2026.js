...
import { WC2026_GROUPS, WC2026_GROUP_FIXTURES, fixtureKickoffUtc } from "./worldCup2026Fixtures.js";
...
export async function clearWorldCup2026Data() {
...
export async function importWorldCup2026Schedule() {
    ...
    const tournament = await pool.query(`INSERT INTO tournaments (external_id, name, season)
    ...
    let teamCount = 0;
    for (const letter of Object.keys(WC2026_GROUPS)) {
        for (const name of WC2026_GROUPS[letter]) {
            const externalId = `wc2026-team-${teamSlug(name)}`;
            const logoUrl = flagUrlForTeam(name);
            const team = await pool.query(`INSERT INTO teams (external_id, name, short_name, logo_url)
        VALUES ($1, $2, $3, $4)
        ON CONFLICT (external_id) DO UPDATE SET
          name = EXCLUDED.name,
          short_name = EXCLUDED.short_name,
          logo_url = EXCLUDED.logo_url
        RETURNING id`, [externalId, name, shortName(name), logoUrl]);
            teamIdByName.set(name, team.rows[0].id);
            teamCount += 1;
        }
    }
    ...
    for (const letter of Object.keys(WC2026_GROUPS)) {
      ...
      RETURNING id`, [tournamentId, `Grupo ${letter}`, `GROUP_${letter}`]);
        ...
        ON CONFLICT (group_id, team_id) DO UPDATE SET
          ...
          rank = EXCLUDED.rank`, [tournamentId, groupId, teamId, rank + 1]);
        }
    }
    let matchCount = 0;
