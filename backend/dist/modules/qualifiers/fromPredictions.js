...
import { computeOfficialQualifiersFromResults, getOfficialQualifiedTeams, OFFICIAL_QUALIFIERS_COUNT, setOfficialQualifiedTeams } from "../scoring/qualifiers.js";
import { recalculateGroupStandings } from "../standings/recalculate.js";
export async function computeUserQualifiersFromPredictions(userId) {
    ...
    for (const g of groupsResult.rows) {
        ...
        const teamsResult = await pool.query(`SELECT s.team_id, t.name FROM standings s
      JOIN teams t ON t.id = s.team_id
      WHERE s.group_id = $1 ORDER BY s.rank`, [groupId]);
        const teamIds = teamsResult.rows.map((r) => r.team_id);
        for (const r of teamsResult.rows) {
            teamNames.set(r.team_id, r.name);
        }
        const matchesResult = await pool.query(`SELECT m.id, m.home_team_id, m.away_team_id,
        p.predicted_home_score, p.predicted_away_score
      FROM matches m
      LEFT JOIN predictions p ON p.match_id = m.id AND p.user_id = $2
      WHERE m.group_id = $1 AND m.stage = 'GROUP'`, [groupId, userId]);
        ...
        groups.push(groupQualificationFromStandings(groupId, groupName, teamIds, matchInputs, teamNames));
    }
    const fifa = resolveFifaQualifiers(groups, teamNames);
    return { ...fifa, predictedMatches, expectedGroupMatches };
}
export async function syncUserQualifierPredictions(userId) {
    ...
    for (const teamId of result.directQualifiers) {
        await pool.query("INSERT INTO qualifier_predictions (user_id, team_id) VALUES ($1, $2) ON CONFLICT DO NOTHING", [userId, teamId]);
    }
    return result;
}
export async function syncAllUsersQualifierPredictions() {
    const users = await pool.query("SELECT id FROM users WHERE role IN ('USER', 'ADMIN')");
    for (const { id } of users.rows) {
        await syncUserQualifierPredictions(id);
    }
    return { users: users.rows.length };
}
export { computeOfficialQualifiersFromResults } from "../scoring/qualifiers.js";
export async function syncOfficialQualifiersFromResults() {
...
export async function enrichQualifiersForApi(userId, fifa) {
    const tournamentId = await getActiveTournamentId();
    const teamsResult = tournamentId
        ? await pool.query(`SELECT t.id, t.name, t.short_name, t.logo_url, g.name AS group_name, g.id AS group_id
        FROM teams t
        JOIN standings s ON s.team_id = t.id AND s.tournament_id = $1
        JOIN groups g ON g.id = s.group_id`, [tournamentId])
        : { rows: [] };
    const teamById = new Map(teamsResult.rows.map((r) => [
        r.id,
        {
            id: r.id,
            name: r.name,
    ...
    WHERE user_id = $1 AND source_type = 'QUALIFIERS' AND source_id = 0`, [userId]);
    return {
...
export async function getOfficialStandingsByGroup() {
    const tournamentId = await getActiveTournamentId();
    if (!tournamentId)
        return [];
    const groups = await pool.query(`SELECT g.id, g.name FROM groups g WHERE g.tournament_id = $1 ORDER BY g.name`, [tournamentId]);
    const result = [];
    for (const g of groups.rows) {
        await recalculateGroupStandings(g.id);
        const teams = await pool.query(`SELECT s.rank, s.points, s.played, s.goal_diff, s.goals_for, s.team_id, t.name, t.logo_url
      FROM standings s JOIN teams t ON t.id = s.team_id
      WHERE s.group_id = $1 ORDER BY s.rank`, [g.id]);
        result.push({
            groupId: g.id,
            groupName: g.name,
