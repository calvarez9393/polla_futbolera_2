...
export async function calculateMatchScores(matchId) {
    ...
    for (const prediction of predictions.rows) {
      ...
      DO UPDATE SET points = EXCLUDED.points, breakdown = EXCLUDED.breakdown, updated_at = NOW()`, [prediction.user_id, matchId, computed.points, JSON.stringify(computed.breakdown)]);
    }
}
