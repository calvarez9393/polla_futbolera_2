...
export async function setOfficialBonusResults(results) {
    await setSetting(OFFICIAL_BONUSES_KEY, JSON.stringify(results));
}
export async function getOfficialBonusResults() {
...
function capPoints(raw, max) {
    return Math.min(raw, max);
}
export async function calculateBonusScores() {
    ...
    for (const row of preds.rows) {
        ...
        const predSemi = row.semifinalist_team_ids ?? [];
        ...
        if (offSemi.length > 0) {
            ...
            if (semiPts > 0)
                breakdown.semifinalists = semiPts;
            points += semiPts;
        }
        const predFinal = row.finalist_team_ids ?? [];
      ...
      DO UPDATE SET points = EXCLUDED.points, breakdown = EXCLUDED.breakdown, updated_at = NOW()`, [row.user_id, points, JSON.stringify(breakdown)]);
        usersScored += 1;
    }
    return { usersScored };
}
