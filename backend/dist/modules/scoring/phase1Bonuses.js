...
import { matchCalendarDateSql, parseCalendarYmd, phase1MatchSql, ymdToExpertDaySourceId } from "../matches/calendarDate.js";
...
function outcomeCorrect(predHome, predAway, realHome, realAway) {
 ...
 */
export async function calculateExpertDayBonuses() {
    const rules = await loadOfficialScoringRules();
    const tournamentId = await getActiveTournamentId();
    if (!tournamentId)
        return { awards: 0, daysProcessed: 0 };
    ...
    const days = await pool.query(`SELECT match_date FROM (
    ...
    for (const { match_date } of days.rows) {
      ...
      ORDER BY m.starts_at`, [tournamentId, ymd]);
        if (matches.rows.length === 0)
            continue;
        const matchIds = matches.rows.map((r) => Number(r.id));
        const users = await pool.query(`SELECT DISTINCT p.user_id
      FROM predictions p
      JOIN users u ON u.id = p.user_id
      WHERE p.match_id = ANY($1::bigint[]) AND u.role IN ('USER', 'ADMIN')`, [matchIds]);
        for (const { user_id } of users.rows) {
            const userId = Number(user_id);
            const preds = await pool.query(`SELECT p.match_id, p.predicted_home_score, p.predicted_away_score,
          m.home_score, m.away_score
        FROM predictions p
        JOIN matches m ON m.id = p.match_id
        WHERE p.user_id = $1 AND p.match_id = ANY($2::bigint[])`, [userId, matchIds]);
            if (preds.rows.length !== matchIds.length)
                continue;
            const allCorrect = preds.rows.every((row) => outcomeCorrect(Number(row.predicted_home_score), Number(row.predicted_away_score), Number(row.home_score), Number(row.away_score)));
            if (!allCorrect)
                continue;
            await pool.query(`INSERT INTO prediction_scores (user_id, source_type, source_id, points, breakdown)
...
export async function calculateInvictoBonuses() {
    const rules = await loadOfficialScoringRules();
    const users = await pool.query(`SELECT id FROM users WHERE role IN ('USER', 'ADMIN')`);
    let usersAwarded = 0;
    for (const { id: userId } of users.rows) {
        const uid = Number(userId);
        const preds = await pool.query(`SELECT p.predicted_home_score, p.predicted_away_score,
      ...
      ORDER BY m.starts_at ASC`, [uid]);
        ...
        for (const row of preds.rows) {
            ...
            if (outcomeCorrect(Number(row.predicted_home_score), Number(row.predicted_away_score), Number(row.home_score), Number(row.away_score))) {
                streak += 1;
      ...
      ON CONFLICT (user_id, source_type, source_id)
      DO UPDATE SET points = EXCLUDED.points, breakdown = EXCLUDED.breakdown, updated_at = NOW()`, [uid, rules.invicto_bonus, JSON.stringify({ invicto: rules.invicto_bonus, maxStreak })]);
        usersAwarded += 1;
    }
    return { usersAwarded };
}
/** Bono maestro de grupo: acertar los 2 clasificados oficiales de un grupo. */
export async function calculateGroupMasterBonuses() {
    const official = await ensureOfficialQualifiedTeams();
    const rules = await loadOfficialScoringRules();
    const groups = await pool.query(`SELECT g.id AS group_id, g.name AS group_name,
    ...
    HAVING COUNT(DISTINCT s.team_id) FILTER (WHERE s.team_id = ANY($1::bigint[])) = 2`, [official]);
    let awards = 0;
    for (const group of groups.rows) {
        const groupId = Number(group.group_id);
        const officialPair = (group.official_team_ids ?? []).map(Number);
        if (officialPair.length !== 2)
            continue;
        const users = await pool.query(`SELECT DISTINCT qp.user_id
      FROM qualifier_predictions qp
      JOIN users u ON u.id = qp.user_id
      WHERE qp.team_id = ANY($1::bigint[]) AND u.role IN ('USER', 'ADMIN')`, [officialPair]);
        for (const { user_id } of users.rows) {
            const userId = Number(user_id);
            const picks = await pool.query(`SELECT team_id FROM qualifier_predictions
        WHERE user_id = $1 AND team_id = ANY($2::bigint[])`, [userId, officialPair]);
            const picked = new Set(picks.rows.map((r) => Number(r.team_id)));
            if (!officialPair.every((id) => picked.has(id)))
                continue;
            await pool.query(`INSERT INTO prediction_scores (user_id, source_type, source_id, points, breakdown)
...
export async function calculateInvictoAndGroupMasterBonuses() {
