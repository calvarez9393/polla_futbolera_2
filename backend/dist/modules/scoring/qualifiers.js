...
export async function setOfficialQualifiedTeams(teamIds) {
    await setSetting(OFFICIAL_QUALIFIERS_KEY, JSON.stringify(teamIds));
}
export async function getOfficialQualifiedTeams() {
...
export async function computeOfficialQualifiersFromResults() {
    const tournamentId = await getActiveTournamentId();
    if (!tournamentId)
        return [];
    const groups = await pool.query(`SELECT id FROM groups WHERE tournament_id = $1 ORDER BY name`, [tournamentId]);
    const teamIds = [];
    for (const g of groups.rows) {
        await recalculateGroupStandings(g.id);
        const top2 = await pool.query(`SELECT team_id FROM standings WHERE group_id = $1 AND rank <= 2 ORDER BY rank`, [g.id]);
 ...
 */
export async function ensureOfficialQualifiedTeams() {
    const saved = await getOfficialQualifiedTeams();
    if (saved.length === OFFICIAL_QUALIFIERS_COUNT)
        return saved;
    ...
    throw new Error(`Se necesitan ${OFFICIAL_QUALIFIERS_COUNT} clasificados oficiales (top 2 por grupo). ` +
...
export async function calculateQualifierScores() {
    ...
    for (const [userId, picks] of byUser) {
      ...
      DO UPDATE SET points = EXCLUDED.points, breakdown = EXCLUDED.breakdown, updated_at = NOW()`, [userId, points, JSON.stringify(breakdown)]);
        usersScored += 1;
    }
    return { usersScored };
}
